...
export const upload = async (req, res) => {
  try {
    ...
    file = req.files.tempFilePath; // here is the field name of the form
    console.log(file);
...
export const register = async (req, res) => {
  try {
    ...
    let user = await Nursery.findOne({ email });

    if (user) {
      return res
        .status(400)
        .json({ success: false, message: "Nursery already exists" });
    }
...
export const updatePassword = async (req, res) => {
  try {
    const user = await Nursery.findById(req.user._id).select("+password");
...
export const forgetPassword = async (req, res) => {
  try {
    ...
    const user = await Nursery.findOne({ email });
...
export const resetPassword = async (req, res) => {
  try {
    ...
    const user = await Nursery.findOne({ resetPasswordOtp: otp });
